...
These strings produce string values with exactly the characters represented. To avoid having escapes, the raw string is prefixed by an `r` and a variable number of `#` characters. The only way for the raw string to be terminated, is by a quote followed by the same quantity of `#` characters. However, this does imply that [raw string literals are context sensitive](https://rust.googlesource.com/rust-lang/rust/+/refs/tags/1.35.0/src/grammar/raw-string-literal-ambiguity.md). This is likely just a theoretical concern, but still one to take into consideration.
